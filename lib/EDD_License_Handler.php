	...
	public function settings( $settings ) {
		$edd_license_settings = array(
			array(
				...
				'name'    => sprintf( __( '%1$s License Key', 'edd', 'testimonials-widget-premium' ), $this->item_name ),
				'desc'    => '',
				'type'    => 'license_key',
...
if ( ! function_exists( 'edd_license_key_callback' ) ) {
	function edd_license_key_callback( $args ) {
		...
		if ( 'valid' == get_option( $args['options']['is_valid_license_option'] ) ) {
			...
			$html .= '<input type="submit" class="button-secondary" name="' . $args['id'] . '_deactivate" value="' . __( 'Deactivate License',  'edd-recurring', 'testimonials-widget-premium' ) . '"/>';
		}
